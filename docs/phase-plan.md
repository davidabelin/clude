...
| 9 | A seat over MCP: a Claude in a chat window plays one seat of a live table through an MCP server on the same registry the screens use | built; 9a-9e deployed, 9f and 9g to deploy. 9a the plan and the renumbering, 2026-09-20. 9b the server and the combined ASGI app, 9c deployed (the secret, the `claude` account, revision 6, probed and live-checked), the connector added and the first live game played from the chat, all 2026-09-21; that game found the views too big for a chat's context, which led to the compact `since` view, `accuse` and `wait` on the answer, and `clude_autopilot`. Then David's four calls the same day: no persona advises a chat seat, tables can be ended (`abandon`, an "End table" button, `tables abandon` on the CLI), a seat that stalls three minutes or is out goes to the stand-in, and no deal until every open seat is taken. 9d the eight fixes after the second live game and 9e David's follow-ups, both 2026-09-22 and deployed that evening (revision `clude-00010-knh`). 9f the chat seat's report after the third live game, 2026-09-25: a move is one line per room answered by naming the room, a 60 s hold under one deadline, and idle replies without the notepad. 9g what a game cost, 2026-09-25: every model call metered to its seat, the cost recorded once the logbook entries are written (on the record, the run summary's Cost column and the table document), one bar per seat tab and a `cost` line for an MCP player, a table budget that holds across midnight UTC, arena runs priced too, and `tables costs` for the older games -- `docs/phase9-plan.md` 8 |
...
- **9b, the server on fake backends (built 2026-09-21; the plan's
  ...
  digest, plus the seat's note. Driver and registry changes:
  `answer(by="mcp")` and a `notes` dict on the document. (9b also built
  an optional `head`, the seat's own agent's fresh numbers; David
  removed it the same day -- no persona advises a chat seat -- so
  `SeatSpec.head` and `WebGame.head_reading` are gone.) Nothing in
  ...
  the view never carrying `readings` or another hand, the floor's
  numbers and nothing else, the note surviving a cold rebuild.
- **9c, serving and one live game (built, deployed and played
  ...
  seat to the floor bot. Then David's four calls: no head, tables
  can be ended, a stalled or out seat goes to the stand-in, no deal
  until every open seat is taken. The plan's section 8 has the
