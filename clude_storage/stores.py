...
class RecordStore(Protocol):
    """What the arena needs from a store. Documents are plain dicts
    (JSON objects) so the store never depends on the record types."""
...
class GcsStore:
    ...
    def _names_under(self, folder: str) -> list:
        prefix = self._full_key(folder).rstrip("/") + "/"
        names = []
